...
Применяем связку Elastik Stack (ELK), где сборщик логов заменен на fluentd. В общем из ELK получается EFK.
...
```
- Посмотрели логи чере kibana и трейсы через zipkin.
